...
- [ ] **Verify the Discord delivery path against a real bot and server.** Needs `DISCORD_BOT_TOKEN`/`DISCORD_GUILD_ID` provisioned first; until then `joinGuild`/`sendDirectMessage` no-op safely. `npm run test-notification -- --discord-id <id>` runs the preflight and renders the DM without sending; `--send` delivers it
