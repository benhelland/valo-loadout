...
- [x] AI vibe-tagging pass — verified live and backfilled: 1318/1365 real skins tagged (the gap is skins synced since the backfill ran, not a failure). `ANTHROPIC_API_KEY` is set (separate Developer-API billing, console.anthropic.com), ~$1.15 actual one-time cost. See `docs/ARCHITECTURE.md` "Color and vibe tagging pipeline" for the prompt-tuning history.
