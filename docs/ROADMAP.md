...
- [ ] **Verify the Discord delivery path against a real bot and server.** Needs `DISCORD_BOT_TOKEN`/`DISCORD_GUILD_ID` provisioned first. Until then `joinGuild`/`sendDirectMessage` no-op safely
