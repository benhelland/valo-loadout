...
- [x] `linked_riot_accounts`/`skin_sighting_stats`/`notifications_sent` schema — already existed; extended this pass with `puuid`, `riotGameName`/`riotTagLine` (so a user can confirm *which* account got linked — the only way to catch a wrong-cookie paste) and `lastError`.
- [x] Account-linking UI with plain-language disclosure — `/account` now has a working link form (paste `ssid`, with step-by-step instructions), live status, "check shop now", the four most recently seen skins, and unlink. Disclosure updated: it now says we never see a password at all, because that's now literally true.
...
- [ ] **Unverified against live Riot endpoints.** Everything above type-checks, lints, unit-tests and runs, but no call has ever been made to Riot — that needs a real `ssid` from a real account, which is the user's to supply. Endpoint shapes came from the community API docs cross-checked against SkinPeek's source; the storefront (v3/POST) is a case where those two disagreed and SkinPeek was right, so treat the rest as likely-but-unproven until a real link succeeds.
