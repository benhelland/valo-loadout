...
**Buddy gallery (`/buddies`):** a plain browse/reference grid of every buddy - denser than the skin grid (more columns, smaller cards) since buddy icons are small and low-res to begin with, and no per-buddy detail page (there's nothing more to show than the name/icon already on the card; picking one for a skin still happens on the skin detail page). Reachable via a small secondary tab pair (`GalleryTabs.tsx`) shown on both `/` and `/buddies`, deliberately not promoted into the site's main top nav - the skin gallery stays the front door for "Gallery".
