...
- At startup that word is compared against `NODE_ENV`, a value tooling sets automatically and a human never types into an env file, so it can't be copy-paste-mismatched the way `DATABASE_URL` can.
