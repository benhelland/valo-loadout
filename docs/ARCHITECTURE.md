...
| **Job trigger** | A scheduled GitHub Actions workflow calling the cron route | The route does the real work (query due accounts, poll Riot, upsert sightings, dispatch notifications); the trigger just wakes it up. |
...
No page or component reads Prisma directly: `src/app/**` and `src/components/**` cannot import the Prisma client at all — `eslint.config.mjs` forbids `@/lib/db` there — so a read a page performs itself lives in `src/queries/` and reaches the page through a function call.

The boundary exists because the render layer is the only one that emits to a browser. A `findMany`/`findUnique`/`findFirst` with no `select` returns every column of the row, and `linked_riot_accounts` holds `encryptedRefreshToken` — the AES-256-GCM ciphertext of a live Riot credential (see `RISKS.md`). A whole-row read in a page component is one careless prop away from serializing that ciphertext into the page's RSC payload; a query function that names its columns cannot make that mistake, because the ciphertext never leaves the database in the first place. The same rule covers a component reaching for a *write*: none currently do (the two inline `"use server"` actions under `src/app/`/`src/components/` call `signIn`/`signOut` only, no Prisma), and if one needed to, the fix is the same as for a read — extract it to `src/actions/`, which is where a mutation belongs regardless of this rule.

`src/queries/payloadBudget.test.ts` enforces the cost side of the same rule across all of `src/`, not just `src/queries/`: a `findMany` must bound rows (`take`) or columns (`select`), and a single-row `findUnique`/`findFirst` must name its columns, or say why with a `// payload-ok:` comment. The cost this bounds is real: `prisma.buddy.findMany()` with neither pulls all 884 buddies with every column into a dropdown that reads two.

**`src/actions/`, `src/store-check/`, `src/notifications/`, `src/sync/` and `src/scripts/` keep their own Prisma access** rather than routing through `src/queries/`, even though the payload rules still bind them. Prisma has no ambient transaction context the way an ORM with a request-scoped session does: `store-check`'s `prisma.$transaction([...])` composes *unexecuted* `PrismaPromise`s, and a layer method that `await`s internally has already run and cannot go in that array — so a shared read layer cannot participate in a transaction without every method taking an explicit `tx` client. `store-check`/`notifications` are also each declared the only seam to Prisma for their subsystem specifically so either can be removed wholesale without touching the rest of the app; routing them through the shared query layer would recouple what that isolation deliberately keeps apart. `src/actions/` is the exception worth naming: most of its reads exist only to validate a write already in flight (an ownership check before a mutation), but `searchSkinsAutocomplete` (`src/actions/search.ts`) is a plain read with no write beside it — it stays a server action, not a `src/queries/` function, because it's called directly from a client component as an action, the same entry point a mutation would use.
...
**The poller runs on Vercel, woken by GitHub Actions.** Requests from Vercel's datacenter IPs reach Riot's store endpoints; Cloudflare does not block them. Two interchangeable triggers wrap the same `runDueShopChecks()`: `GET /api/cron/check-shops` (Bearer `CRON_SECRET`, constant-time compare, fails closed with 503 if unconfigured) and `npm run check-shops` (runs anywhere, including a residential connection). The second stays as the fallback should that ever change: running the job somewhere it isn't blocked is not evasion; rotating IPs to dodge a block would be.

**The schedule** (`.github/workflows/check-shops.yml`) fires at 00:05 and 00:40 UTC. The store reset is a fixed moment per Riot region (00:00 UTC for `na`), and each account's `nextPollAt` is that reset plus one minute plus up to 30 minutes of jitter. The first run catches early jitter; the second catches the rest and GitHub's own start-up lag. Riot is called once per account per day however often the route is woken. Another region's reset would appear in its accounts' `nextPollAt`, and the schedule gains a line. Vercel Cron is not used: the Hobby tier allows only once-daily expressions with no control over the minute, and the route has to run shortly *after* the reset. The `/account` page's "check shop now" control covers the manual case.
...
- **The DM is one embed per matched skin**, capped at four. Discord allows one thumbnail per embed, so a single embed cannot show four skin renders. Each embed's title links to that skin's detail page, carries its render as the thumbnail and its VP price when one is known, and names the Riot account in the footer. A skin with no icon degrades to a text-only embed rather than a null thumbnail, which Discord rejects outright. `src/notifications/messages.ts` builds the payload as pure functions and applies Discord's length caps; `messages.test.ts` asserts the result stays inside them, because an oversized embed fails as a silent 400.
- **Dedupe is keyed on the linked account as well as the user and skin**, over a rolling ~20h window — roughly one shop cycle. With several accounts linked, the same wishlisted skin appearing in two different shops is two things worth knowing, not a repeat. `pruneNotificationHistory` runs alongside the stale-link sweep and drops rows older than twice that window; the doubling is the margin that keeps a prune from racing a live dedupe read. There is no blast-radius guard of the kind `expireStaleLinks` carries, deliberately: those rows are bookkeeping, and over-deleting costs one duplicate DM rather than a destroyed credential.
- **`users.wishlist_notifications_enabled` mutes wishlist DMs**, checked before the wishlist read so a muted user costs one cheap query. It does not mute the expiry warning, which is transactional rather than noise.
- **`/account` checks guild membership on load** (`isGuildMember`, one call through the same seam) and says so when the user is not in the server, since a DM cannot reach them until they sign in again. Null when delivery is unconfigured or Discord is unreachable, so an outage never shows a false warning.
- **Every send records its outcome** on `users.notification_failed_at` / `notification_failure_reason`, cleared by the next delivery that succeeds, so `/account` can say a DM failed instead of silently never arriving. `NOT_CONFIGURED` is never recorded — a deployment with no bot token is the operator's problem, and recording it would put a banner on every account.

`npm run test-notification -- --discord-id <id>` renders either DM from real catalog rows and prints it; `--send` delivers it. Its preflight checks that the recipient is a guild member, which is what predicts a 50007 before it happens. It is a probe: it writes no dedupe rows, touches no failure columns, and ignores the opt-out.
