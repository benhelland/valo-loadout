...
**Buddy-on-weapon rendering:** buddies attach as a 2D overlay at a per-weapon anchor point (roughly where the strap/grip is), composited onto the skin's flat render only — never onto video/gameplay footage, since a fixed-percentage anchor only makes sense against a static image (an earlier version of this got that wrong and overlaid on whatever media was active, including video, which looked broken). Lives in its own UI panel separate from the main level/chroma/animation preview, not layered on top of it. Anchor points are a small static config (`weapon_id -> {x, y, scale}`, `src/lib/buddyAnchors.ts`), not a DB table, since they're a design constant per weapon, not user data. Set ceiling expectation: this is a 2D sticker-on-a-photo approximation (both the weapon render and the buddy icon are flat, fixed-angle images, not 3D), not a true composite — see the declined-3D note above for why that's the actual ceiling of this approach. One generalized default anchor currently covers all weapons reasonably well (checked against a rifle, a sniper, and a pistol); per-weapon overrides remain easy to add in that file as they get tuned further.
