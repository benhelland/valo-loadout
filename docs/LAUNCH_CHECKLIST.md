...
Mechanical action items for deploying this app for real — distinct from `ROADMAP.md`, which tracks feature phases rather than launch-day steps.
...
- [ ] Confirm `AUTH_URL` (or Auth.js's auto-detection via request headers) resolves correctly behind Vercel's proxy. Auth.js v5 usually auto-detects via `x-forwarded-host`; verify with a real sign-in round-trip on the deployed domain before calling this done.
...
- [ ] Verify end-to-end on the deployed app: sign in with a real Discord account, confirm it lands in the configured guild, wishlist a skin that's in that account's current shop (or trigger a check right after linking), and confirm the DM arrives. This path has not yet been tested against a real bot and server (see `ROADMAP.md`).
...
- [ ] Sanity-check that a linked account survives a deploy. The encrypted token lives in Postgres rather than in any instance's memory, so it should — confirm it does.
