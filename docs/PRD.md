...
2. **Loadout builder** — pick a skin (and level, chroma, buddy) for every weapon slot to assemble a full "ideal loadout," save it, revisit and edit it. Each filled slot shows its skin thumbnail with the buddy as a small badge on it (same badge treatment as the skin detail page), not composited onto the weapon render.
   ...
   - **Board layout matches the real client's Collection screen** — category tabs (Sidearms/SMGs/Shotguns/Rifles/Snipers/Heavy/Melee), a row of weapons within the active category, one large focused view of whichever weapon is selected (not a grid of all 20 slots at once — that was the original plan, tried, and replaced once the real screen's actual "one weapon at a time" navigation was confirmed). The focused view's "Change Skin" / "Assign Skin" action opens the picker: the gallery's filter/search UI (including color/vibe) scoped to that weapon, so building a loadout still feels like an extension of browsing, not a separate form.
