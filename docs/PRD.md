...
1. **Skin gallery** — every skin ever released for every weapon and knife, plus every buddy, browsable and filterable (by weapon, rarity/tier, collection, price, release date). Each skin gets a real showcase: high-res images, animation/video where the underlying data provides it, and polished presentation (zoom, smooth transitions) — not a static icon grid.
   ...
   - **Filters:** weapon/category, rarity tier (with Riot's own tier icon), collection/bundle, price (VP — **real** where Riot has quoted it, narrowly estimated from rarity otherwise, and shown as Unknown when neither applies; see `ARCHITECTURE.md` "Real VP prices"), "has animation," **color** (dominant color family, extracted per skin and per chroma — a skin with multiple color variants can match several color filters), and **vibe** (dark, sleek, futuristic, elegant, aggressive, cute, retro, neon, anime, etc. — AI-assigned per skin at sync time, see `ARCHITECTURE.md`). Free-text search always available. Sort: rarity (default), price, alphabetical. **No release-date sort or filter** — no source provides real release dates, and the timestamp we do have is an insert time whose entire launch catalog lands in one 15-second window; see `ARCHITECTURE.md` for the measurement and for what replaces the "what's new" intent instead.
