...
1. **Skin gallery** — every skin ever released for every weapon and knife, plus every buddy, browsable and filterable (by weapon, rarity/tier, collection, price, release date). Each skin gets a real showcase: high-res images, animation/video where the underlying data provides it, and polished presentation (zoom, smooth transitions) — not a static icon grid.
   ...
   - **Filters:** weapon/category, rarity tier (with Riot's own tier icon), collection/bundle, price (VP — **real** where Riot has quoted it, narrowly estimated otherwise, and shown as Unknown when neither applies; see `ARCHITECTURE.md` "Pricing"), "has animation," **color** (dominant color family, extracted per skin and per chroma — a skin with multiple color variants can match several color filters), and **vibe** (dark, sleek, futuristic, elegant, aggressive, cute, retro, neon, anime, etc. — AI-assigned per skin at sync time). Free-text search always available. Sort: rarity (default), price, alphabetical. **No release-date sort or filter** — no source provides real release dates, and the timestamp we do have is an insert time whose entire launch catalog lands in one 15-second window; see `ARCHITECTURE.md` for what replaces the "what's new" intent instead.
   - **Skin detail page:** levels (a skin's upgrade tiers — visual/audio/finisher unlocks) as a selector, chromas as color swatches that swap the preview instantly, and a buddy picker shown as a badge in the preview frame rather than composited onto the weapon render (see `ARCHITECTURE.md` → "Buddies").
   - **Performance:** the catalog is large (1500+ skins across years), so the grid is paginated with a capped page size, thumbnails lazy-load, and video plays on open rather than autoplaying across the grid.
2. **Loadout builder** — pick a skin (and level, chroma, buddy) for every weapon slot to assemble a full "ideal loadout," save it, revisit and edit it. Each filled slot shows its skin thumbnail with the buddy as a small badge on it (same badge treatment as the skin detail page), not composited onto the weapon render.
   ...
   - **Aspirational, not inventory-gated** — a user can pick any skin/level/chroma regardless of whether they actually own it. This is a planning tool for the target user who plans purchases, not an owned-items tracker. (A separate "my collection" owned-skins view is deferred — see `ROADMAP.md` Phase 4. It needs a different Riot endpoint and its own data-minimization stance, which is real design work rather than a small add-on.)
   - **Board layout** — a 4-column grid grouped by category (Sidearms | SMGs+Shotguns | Rifles+Melee | Snipers+Heavy), every slot visible at once like a locker or armory. Each tile shows its assigned skin render with the buddy as a small icon docked beside it, and the weapon name in a label bar below. Clicking a filled slot pops up a "View / Replace" choice (View jumps to that exact skin+level+chroma+buddy combo via a `/combo/:encoded` link, so the picked variant is preserved rather than the skin's defaults; Replace opens the picker); clicking an empty slot goes straight to the picker. The picker is the gallery's own filter/search UI scoped to that weapon, so building a loadout feels like an extension of browsing rather than a separate form.
...
5. **Store notifications** — link a Riot account; when a wishlisted skin appears in that account's daily shop, notify the user. This is the feature that makes the wishlist actually useful instead of just a list.
   - **Channel (v1): Discord bot DM, automatic off the user's existing Discord sign-in — not a manually-configured webhook.** Fits the audience (this space already lives in Discord — see competitive landscape below), needs no email infrastructure, and needs no setup step beyond the signup and Riot-link the user already does — see `ARCHITECTURE.md` → "Notifications subsystem" for why a webhook URL couldn't meet that bar. Email and push are deferred, not designed away.
...
6. **Account linking** — connect a Riot account for the notification feature. Must be clearly opt-in, with plain-language disclosure of what this does and the (small but real) risk profile — see `RISKS.md`.
   - **No password, and therefore no 2FA step.** Linking goes through Riot's own OAuth sign-in page; the app never receives a password and never triggers a two-factor challenge. See `ARCHITECTURE.md` → "Store-check subsystem".
   - **Honest failure states:** if Riot's protections block an account, the user needs a clear "this account can't be linked right now" message, not a silent retry loop or a generic error.
