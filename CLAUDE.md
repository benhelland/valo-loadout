...
**After the first `prisma migrate deploy` against any new database** (a fresh dev setup, or a new production database), run `npx tsx src/scripts/setEnvironmentMarker.ts <development|production>` against it once. The app refuses to start if `NODE_ENV` and this marker ever disagree (`src/instrumentation.ts` → `src/lib/verifyEnvironment.ts`) — it's what turns a copy-pasted-into-the-wrong-place `DATABASE_URL` into an immediate crash instead of silently mixed data. Skipping this step just means the check warns once and does nothing, not a hard failure — safe to defer, not safe to forget indefinitely on a real production database. Also least-privilege the app's own `DATABASE_URL` on any new database with `src/scripts/setupAppRole.ts` — see `docs/RISKS.md` "What a database breach would actually expose" for why the app shouldn't connect as the Neon owner role.
