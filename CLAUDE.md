...
**Status: Phase 1 + 2 built (not deployed); Phase 3 store-check built but never run against live Riot** (see `docs/ROADMAP.md`). The gallery, loadout builder, sharing, real Discord auth, and the Riot store-check subsystem all exist and are verified as far as they can be locally. Two things are outstanding and worth knowing before picking anything up: **(1)** nothing has been deployed yet — deployment was deferred deliberately, see `docs/LAUNCH_CHECKLIST.md`; **(2)** the store-check subsystem has never made a real call to Riot, because that needs a real account session the user must supply. Remaining feature gaps: the wishlist (Phase 2) and, dependent on it, notification dispatch. See `docs/` for the full thinking. Do not assume any framework, package, or file structure beyond what's written down in these docs — propose changes to the docs themselves before writing code that contradicts them.
...
- **…but specifically via cookie reauth, never a password.** The originally-documented username/password flow was checked against the live API and is dead: it now requires an hCaptcha token, so it's CAPTCHA-gated for every account. The app never receives a password; the user pastes an `ssid` session cookie which is validated, encrypted (AES-256-GCM) and used with Riot's cookie-reauth endpoint. Don't "restore" the password flow — it can't work without solving a CAPTCHA, which this project won't do. Corollary: `PRD.md`'s 2FA/code-entry requirement is moot, since we never trigger a 2FA challenge.
...
- `npm test` — unit tests via Node's built-in runner (no Jest/Vitest dependency). Currently covers `src/lib/crypto.ts` and `src/riot/ssid.ts` — the two security controls where a silent regression would be worst. Add tests for logic in that category; don't chase coverage on UI or glue code
